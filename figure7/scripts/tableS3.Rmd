...
subtitle: "Markdown to reproduce Table S3"
author:  
...
```{r}
# statistical testing 
data_stat_test <- norm_data %>% 
        filter(treatment == "medium" & cell_line != "HEK293T_wt") %>%
        select(cell_line,experiment,plate_id,norm_punct_count) %>% 
        unite(replicate,experiment,plate_id,sep=":") %>% 
        as.data.frame()

res_stat_test <- pairwise.wilcox.test(data_stat_test$norm_punct_count,
                                      data_stat_test$cell_line,
                     p.adjust.method = "BH")
